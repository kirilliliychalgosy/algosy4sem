    ...
    for (size_t cut_pos = 0; cut_pos < input_size; ++cut_pos) {
        ...
        if (((left_or_right) && (checked_part_is_palindrom)) ||
            ...
            if ((if_complement_exists != input_compact_form.end()) && (index != if_complement_exists->second)) {
                ...
                if (left_or_right) {
                    indices_pairs.push_back(std::pair<size_t, size_t>(index, if_complement_exists->second));
                } else {
                    indices_pairs.push_back(std::pair<size_t, size_t>(if_complement_exists->second, index));
                }
            }
        }
    }
